    ...
    <div class="product-layout">
      <div class="product-image-col">
        <div class="product-image-wrap">
          ${product.image
            ? `<img id="main-img" src="${product.image}" alt="${name}" class="lb-trigger" style="cursor:zoom-in"
        ...
        ${product.image ? `
          <div class="product-thumbs">
            <div class="product-thumb active" data-src="${product.image}">
              <img src="${product.image}" alt="${name}" loading="lazy">
            </div>
            <div class="product-thumb" data-src="${product.image}" style="filter:sepia(0.3)brightness(1.1)">
              <img src="${product.image}" alt="${name}" loading="lazy" style="filter:sepia(0.3)brightness(1.1)">
            </div>
            <div class="product-thumb" data-src="${product.image}" style="filter:contrast(1.1)saturate(1.2)">
              <img src="${product.image}" alt="${name}" loading="lazy" style="filter:contrast(1.1)saturate(1.2)">
            </div>
          </div>
        ` : ''}
      </div>
        ...
        ${product.sizes?.length ? `
          <div class="size-selector">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem">
              <label style="font-size:0.75rem;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--text-light)" data-lang-key="product_select_size">Select Size</label>
              <button class="size-guide-link" id="size-guide-btn">Size Guide</button>
            </div>
...
function bindSizeButtons() {
  document.querySelectorAll('.size-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const barSize = bar?.querySelector('.sticky-bar-size');
      if (barSize) barSize.textContent = selectedSize;
    });
  });
}
  ...
  document.querySelectorAll('.product-thumb').forEach(thumb => {
    thumb.addEventListener('click', () => {
      ...
      if (mainImg) {
        ...
        mainImg.src = src;
        mainImg.onclick = () => window.__openLightbox(src, currentProduct?.nameEn || '');
      }
    });
  });
}

function bindSizeGuide(product) {
...
function initStickyBar(product) {
  ...
  bar.innerHTML = `
    <div class="sticky-bar-info">
      <span class="sticky-bar-name">${name}</span>
      <span class="sticky-bar-price">$${product.price.toLocaleString()} · <span class="sticky-bar-size">${selectedSize}</span></span>
    </div>
    <a id="sticky-order-btn" class="btn-whatsapp" href="#" target="_blank" rel="noopener" style="animation:none">
  ...
  function updateStickyLink() {
    if (stickyBtn && currentProduct) {
      const msg = `Hi, I'm interested in "${currentProduct.nameEn}" Ref: ${currentProduct.ref} in size ${selectedSize || 'any'}. Promo: ${PROMO_CODE}`;
      stickyBtn.href = `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(msg)}`;
    }
  }
  ...
  document.addEventListener('product-size-change', () => {
    const barSize = bar.querySelector('.sticky-bar-size');
    if (barSize) barSize.textContent = selectedSize;
    updateStickyLink();
  });
...
function updateOrderLink() {
  ...
  const msg = `Hi, I'm interested in "${currentProduct.nameEn}" Ref: ${currentProduct.ref} in size ${selectedSize || 'any'}. Can I get more info? Promo: ${PROMO_CODE}.`;
